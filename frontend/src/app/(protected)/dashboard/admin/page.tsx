  ...
  return (
    <section className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-xs md:text-sm text-slate-400">Platform overview, system load auditing, active workspace telemetry, and quick tasks.</p>
      </div>
                  ...
                  {data.recentProjects.slice(0, 4).map((p) => (
                    <Card key={p.id} className="p-5 flex flex-col justify-between hover:bg-slate-800/40 h-full min-w-0">
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400 gap-3 min-w-0">
                        <span className="font-medium text-slate-300 truncate" title={p.manager?.name || "Unassigned"}>{p.manager?.name || "Unassigned"}</span>
